  ...
  return (
    <section className="my-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative group/carousel">
        {currentIndex > 0 && (
          <button onClick={scrollLeft} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 bg-black/70 p-2 rounded-full z-10">
            <img src={leftArrowSrc} alt="Prev" className="w-6 h-6" />
          </button>
        )}
        ...
        {currentIndex < items.length - itemsPerView && (
          <button onClick={scrollRight} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 bg-black/70 p-2 rounded-full z-10">
            <img src={rightArrowSrc} alt="Next" className="w-6 h-6" />
          </button>
        )}
      </div>
    </section>
